...
def infer_and_visualize(model, loader, device, num_images=10):
    ...
    if isinstance(model,Conv_Autoencoder):
    ...
    if isinstance(model,Conv_Autoencoder):
        ...
        vis_reconstructed = reconstructed.cpu()
    else:
        vis_images = images.view(-1,1,28,28).cpu()
        vis_reconstructed = reconstructed.view(-1, 1, 28, 28).cpu()        
